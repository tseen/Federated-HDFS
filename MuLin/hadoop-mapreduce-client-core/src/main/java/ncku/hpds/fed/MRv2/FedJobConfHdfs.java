...
public class FedJobConfHdfs extends AbstractFedJobConf {
	...
	private static String FS_DEFAULT_NAME_KEY = "fs.defaultFS";
	...
	public FedJobConfHdfs(Configuration jobConf, Job job, String inputName)
		...
		if (Inputs.length > 0) {
			for (int i=0; i<Inputs.length ; i++){
				System.out.println("*Input path " + i + " :"+Inputs[0].toString());
			}
			String p[] = Inputs[0].toString().split("/");
			// p[] = hdfs://10.2.0.1:20500/user/hpds/input
