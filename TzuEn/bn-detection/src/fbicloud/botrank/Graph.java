...
public class Graph<T> {
	...
	public int edgeCount() {
		return adjacencyList.values().stream().mapToInt(Node::getEdgeCount)
				.sum();
	}
	...
	public static void main(String[] args) throws FileNotFoundException,
		...
		for (int i = 0; i < Integer.parseInt(args[1]); i++) {
			int randomNum = rand.nextInt(Integer.parseInt(args[2])) + 1;
			for (int j = 0; j < randomNum; j++) {
				int randomNeighbor = rand.nextInt(Integer.parseInt(args[1])) + 1;
				graph.addEdge(Integer.toString(i+1),
						Integer.toString(randomNeighbor));
			}

		}

		for (int i = 0; i < Integer.parseInt(args[1]); i++) {
			Node<String> n = graph.getNode(Integer.toString(i+1));
			List<Edge<String>> edgeList = n.edges();
			//int nCount = 0;
			writer.print(i+1);
			writer.print("	125	");
			for (Edge<String> edge : edgeList) {
				//nCount ++;
				writer.print(edge.toNode().vertex()+":");
				//writer.println(edge.fromNode().vertex()+","+edge.toNode().vertex()+"\t"+"0.0");				
			}
			writer.print("\n");
		}
