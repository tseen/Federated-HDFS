...
public class GenericProxyReducer<T1, T2> extends Reducer<T1, T2, Text, Text> {
	...
	private static double MB_FACTOR = 0d;
	private static double VCORES_FACTOR = 0d;
	...
	public void setup(Context context) throws UnknownHostException {
		...
		if(conf.get("wanOpt").equals("true")){
			...
			for(int i = 0; i<nodes.length; i++){
					...
					System.out.println("Fomula " + nodes[i].split("=")[0] +"= " + resources[0] + "+" + resources[1] + "+" + resources[2]+ "+" + resources[3]);
					infoGain = WAN_FACTOR * Double.parseDouble(resources[0]) 
							        //+ NODES_FACTOR * Double.parseDouble(resources[0])
							        + MB_FACTOR * Double.parseDouble(resources[0])
							        + VCORES_FACTOR * Double.parseDouble(resources[1])
							        + MAP_INPUTSIZE_FACTOR * Double.parseDouble(resources[2]); 
