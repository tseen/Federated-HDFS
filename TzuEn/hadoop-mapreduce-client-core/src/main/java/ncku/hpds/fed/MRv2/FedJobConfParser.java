    ...
    public void parse() {
        ...
        try {
            ...
            if ( topCloudNode.getNodeType() == topCloudNode.ELEMENT_NODE ){
                ...
                mTopCloudConf.setHadoopHome(
                        getTagValue("HadoopHome", topCloudElement ,INVALID_VALUE));
            ...
            for ( int i = 0 ; i < regionCloudList.getLength(); i++ ) {
                ...
                if ( regionCloudNode.getNodeType() == Node.ELEMENT_NODE ) {
                    ...
                    if(i==0){
                    	paths = fedHadoopConf.getName() + "_" + fedHadoopConf.getJobName()+"_OUT_"+ mFedJobSubmittedTime;
                    }
                    else
                    	paths += ","+fedHadoopConf.getName() + "_" + fedHadoopConf.getJobName()+"_OUT_"+ mFedJobSubmittedTime;
                    ...
                    for ( int j = 0 ; j < 10 ; j++ ) {
                        ...
                        mTopCloudConf.setArgs( j, getTagValue(argTag, regionCloudElement ,INVALID_VALUE));

                    }
                    fedHadoopConf.setOtherArgs(
                            getTagValue("OtherArgs", regionCloudElement ,INVALID_VALUE));
                    mRegionCloudList.add(fedHadoopConf);
                    mTopCloudConf.setOtherArgs(
                            getTagValue("OtherArgs", regionCloudElement ,INVALID_VALUE));
               } 
            }
            // path style: path1,path2,path3...
            mTopCloudConf.setHDFSInputPath(paths);
        } catch ( Exception e ) {
            e.printStackTrace();
        }
    }
