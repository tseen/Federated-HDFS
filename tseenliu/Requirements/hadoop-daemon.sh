...
case $startStop in
  ...
  (start)
    ...
    cd "$HADOOP_PREFIX"
    case $command in
      supernamenode|namenode|secondarynamenode|datanode|journalnode|dfs|dfsadmin|fsck|balancer|zkfc)
        if [ -z "$HADOOP_HDFS_HOME" ]; then
          hdfsScript="$HADOOP_PREFIX"/bin/fedhdfs
        else
          hdfsScript="$HADOOP_HDFS_HOME"/bin/fedhdfs
        fi
